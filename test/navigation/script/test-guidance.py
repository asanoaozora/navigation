...
* \author Philippe Colliot <asanoaozora@outlook.com>*
* \version 1.1
*
...
def guidance_guidanceStatusChanged_handler(guidanceStatus, routeHandle):
    ...
    if guidanceStatus != genivi.ACTIVE and g_guidance_active is True:
        g_guidance_active = False
        route = g_current_route + 1
        if route < g_routes.length:
            launch_route_calculation(route)
        else:
            for i in range(g_routes.length):
                g_routing_interface.DeleteRoute(dbus.UInt32(g_navigationcore_session_handle), dbus.UInt32(g_routes[i].getElementsByTagName("handle")[0].childNodes[0].data))
...
def launch_guidance(route):
    ...
    g_mapviewercontrol_interface.SetTargetPoint(dbus.UInt32(g_mapviewer_sessionhandle),
                                                dbus.UInt32(g_mapviewer_maphandle),
                                                dbus.Struct((
                                                    dbus.Double(locations[g_routes[g_current_route].getElementsByTagName("start")[0].childNodes[0].data][0]),
                                                    dbus.Double(locations[g_routes[g_current_route].getElementsByTagName("start")[0].childNodes[0].data][1]),
                                                    dbus.Double(0)
                                                )))
...
def launch_route_calculation(route):
    ...
    print('Route name: ' + g_routes[g_current_route].getElementsByTagName("name")[0].childNodes[0].data)
    # get route handle
    ret = g_routing_interface.CreateRoute(dbus.UInt32(g_navigationcore_session_handle))
    g_route_handle = ret[1]
    g_routes[g_current_route].getElementsByTagName("handle")[0].childNodes[0].data = g_route_handle
    print('Route handle: ' + str(g_route_handle))
    start = g_routes[g_current_route].getElementsByTagName("start")[0].childNodes[0].data
    dest = g_routes[g_current_route].getElementsByTagName("destination")[0].childNodes[0].data
    print('Calculating route from '
        + start
        + '(' + str(locations[g_routes[g_current_route].getElementsByTagName("start")[0].childNodes[0].data][0])
        + ', ' + str(locations[g_routes[g_current_route].getElementsByTagName("start")[0].childNodes[0].data][1])
        + ') to' + dest
        + '(' + str(locations[g_routes[g_current_route].getElementsByTagName("destination")[0].childNodes[0].data][0])
        + ', ' + str(locations[g_routes[g_current_route].getElementsByTagName("destination")[0].childNodes[0].data][1]) + ')')
    # set waypoints
    ...
    g_routing_interface.SetWaypoints(dbus.UInt32(g_navigationcore_session_handle),
                                    ...
                                    dbus.Boolean(0),
                                    dbus.Array([
                                        dbus.Dictionary({dbus.Int32(genivi.LATITUDE): dbus.Struct([dbus.Byte(waypointDoubleCapiType),
                                            dbus.Double(locations[g_routes[g_current_route].getElementsByTagName("start")[0].childNodes[0].data][0])]),
                                            dbus.Int32(genivi.LONGITUDE): dbus.Struct([dbus.Byte(waypointDoubleCapiType),
                                            dbus.Double(locations[g_routes[g_current_route].getElementsByTagName("start")[0].childNodes[0].data][1])])}),
                                        dbus.Dictionary({dbus.Int32(genivi.LATITUDE): dbus.Struct([dbus.Byte(waypointDoubleCapiType),
                                            dbus.Double(locations[g_routes[g_current_route].getElementsByTagName("destination")[0].childNodes[0].data][0])]),
                                            dbus.Int32(genivi.LONGITUDE): dbus.Struct([dbus.Byte(waypointDoubleCapiType),
                                            dbus.Double(locations[g_routes[g_current_route].getElementsByTagName("destination")[0].childNodes[0].data][1])])})
                                    ])
    )
...
if __name__ == '__main__':
    ...
    g_routes = route_set.getElementsByTagName("route")
